...
fn set_sprite<E: Clone + Reflect + Debug>(
    path: &'static str,
) -> impl Fn(On<Pointer<E>>, Query<&mut CxSprite>, Res<AssetServer>) {
    move |trigger, mut sprites, assets| {
        **sprites.get_mut(trigger.entity).unwrap() = assets.load(path);
    }
}

fn init(mut cursor: ResMut<CxCursor>, assets: Res<AssetServer>, mut cmd: Commands) {
    cmd.spawn(Camera2d);
    ...
    *cursor = CxCursor::Filter {
    ...
    cmd.spawn((CxPosition(ivec2(8, 8)), CxSprite(assets.load(idle_path))))
