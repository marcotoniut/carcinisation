...
use crate::{
    stage::{
        attack::components::*,
        components::interactive::{Collision, Hittable},
        events::DamageEvent,
        player::components::{
...
pub fn check_got_hit(
    ...
    mut hittable_query: Query<(Entity, &PxSubPosition, &Collision), With<Hittable>>,
    mut score: ResMut<Score>,
) {
    let camera_pos = camera_query.get_single().unwrap();
    for (attack, mut hit_list) in attack_query.iter_mut() {
        for (entity, position, collision) in hittable_query.iter_mut() {
