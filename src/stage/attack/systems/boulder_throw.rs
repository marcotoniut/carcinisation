...
use crate::{
    ...
    stage::{
        ...
        enemy::{components::EnemyAttack, data::boulder_attack::BOULDER_ATTACK_ANIMATIONS},
        events::DamageEvent,
...
// TODO simplify
pub fn boulder_throw_damage_on_reached(
    ...
    for (entity, damage, position, depth) in &mut depth_query.iter() {
        ...
        let animation_o = BOULDER_ATTACK_ANIMATIONS.hit.get(&depth.0);
        if let Some(animation) = animation_o {
            commands.spawn((
                Name::new("Attack - Boudler throw - hit"),
