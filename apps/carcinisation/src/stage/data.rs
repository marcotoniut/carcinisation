...
pub struct StageData {
    ...
    /// Individually authored primitives spawned at stage startup.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub primitives: Vec<StagePrimitive>,
    /// Optional generated primitive bands (e.g. depth-visualisation ground).
    /// Produces additional primitives at startup alongside `primitives`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub primitive_bands: Option<PrimitiveBandConfig>,
}
...
/// A positioned primitive shape declared in stage data.
///
/// Spawned as a [`CxPrimitive`] entity at stage startup. Can be authored
/// individually in RON or generated by helpers like [`PrimitiveBandConfig`].
#[derive(Clone, Debug, Deserialize, Reflect, Serialize)]
pub struct StagePrimitive {
    pub shape: StagePrimitiveShape,
    pub fill: StagePrimitiveFill,
    /// Position in logical pixels. Interpretation depends on `camera_space`.
    pub position: IVec2,
    /// Anchor point for the primitive.
    #[serde(default)]
    pub anchor: StagePrimitiveAnchor,
    /// When true, position is screen-relative (camera space).
    /// When false, position is world-relative.
    #[serde(default)]
    pub camera_space: bool,
    /// Render layer name. Defaults to Background.
    #[serde(default)]
    pub layer: StagePrimitiveLayer,
}

/// Shape for a stage primitive.
#[derive(Clone, Debug, Deserialize, Reflect, Serialize)]
pub enum StagePrimitiveShape {
    Rect { width: u32, height: u32 },
}

/// Fill for a stage primitive.
#[derive(Clone, Debug, Deserialize, Reflect, Serialize)]
pub enum StagePrimitiveFill {
    Solid(u8),
    Checker { a: u8, b: u8 },
    OrderedDither { a: u8, b: u8, threshold: u8 },
}

/// Anchor for a stage primitive.
#[derive(Clone, Copy, Debug, Default, Deserialize, Reflect, Serialize)]
pub enum StagePrimitiveAnchor {
    TopLeft,
    TopCenter,
    #[default]
    BottomCenter,
...
/// Layer for a stage primitive.
#[derive(Clone, Copy, Debug, Default, Deserialize, Reflect, Serialize)]
pub enum StagePrimitiveLayer {
    Skybox,
    #[default]
    Background,
}

/// Configuration for generating horizontal primitive rect bands across the
/// depth range.  Authored in RON, consumed at stage startup to spawn
/// [`CxPrimitive`] entities.
#[derive(Clone, Debug, Deserialize, Reflect, Serialize)]
pub struct PrimitiveBandConfig {
    /// Palette index for the skybox fill (drawn above the horizon).
    pub skybox_color: u8,
    /// Ordered list of palette indices for ground bands, from the nearest
    /// (foreground) to the farthest (just below the horizon).
    /// Transitions between adjacent colours use the configured `transition`.
    pub ground_colors: Vec<u8>,
    /// How adjacent ground colours are blended.
    #[serde(default)]
    pub transition: BandTransition,
    /// Width multiplier relative to the viewport width.
    /// `2.0` = 200% of viewport width, centred.
    #[serde(default = "default_width_multiplier")]
    pub width_multiplier: f32,
}

/// How adjacent ground-colour bands blend into each other.
#[derive(Clone, Copy, Debug, Default, Deserialize, Reflect, Serialize)]
pub enum BandTransition {
    /// Alternating solid colour and checker-pattern transition bands.
    #[default]
    Checker,
    /// Alternating solid colour and ordered-dither transition bands.
    OrderedDither {
        /// Dither coverage level (0..=16).  8 = half coverage.
        threshold: u8,
    },
}
...
mod tests {
    ...
    #[test]
    fn gallery_stage_ron_deserializes() {
        let ron_bytes = include_str!("../../../../assets/stages/gallery.sg.ron");
        let data: StageData =
            ron::from_str(ron_bytes).expect("gallery.sg.ron should deserialize successfully");
        assert_eq!(data.name, "Gallery");
        let bands = data
            .primitive_bands
            .expect("debug_depth_bands should have primitive_bands");
